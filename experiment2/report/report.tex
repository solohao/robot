...
\begin{enumerate}[label=\arabic*)]
  ...
  TurtleBot4，完成现场地图、初始位姿、目标点、安全边界和运行数据采集。本部分
  当前尚未实施，报告只提供可直接填写的实验模板，不虚构任何实测数据。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*)]
  ...
  \item 为实地实验预留设备、网络、安全、路线、数据和异常记录模板。
\end{enumerate}
...
\subsection*{4.6 实地实验操作模板}

实地实验尚未执行。后续应按以下顺序进行，并在每一步完成后填写表格：
\begin{enumerate}[label=\arabic*)]
  \item 检查机器人电量、急停、轮胎、激光雷达和安全隔离区；
  \item 记录 TurtleBot4 与远程主机的网络、DDS domain 和时间同步配置；
  \item 在低速手动模式下验证 \texttt{/scan}、\texttt{/odom}、TF 和
  \texttt{/cmd\_vel}；
  \item 建图并保存实地地图，记录地图分辨率和尺寸；
  \item 在地图中标定实地起点、目标点、障碍物与安全距离；
  \item 先以低速和短路线验证，再执行完整路线；
  \item 同步保存 bag、路径、action、最终位姿、照片和连续视频；
...
\begin{longtable}{p{4.3cm}p{4.4cm}p{4.4cm}}
  ...
  \endhead
  实验日期、地点 & \pending & \\
  TurtleBot4 型号、序列号 & \pending & \\
  ROS 2 / 固件版本 & \pending & \\
  网络、DDS domain、主机 IP & \pending & \\
  地图文件、分辨率、尺寸 & \pending & \\
  ...
  直线距离、绕行比 & \pending & \\
  最小障碍距离 & \pending & \\
  action 终态、耗时 & \pending & \\
  最终位置误差、航向误差 & \pending & \\
  重复次数、成功次数 & \pending & \\
  bag、照片、视频文件名 & \pending & \\
  异常与人工接管记录 & \pending & \\
  \bottomrule
\end{longtable}
...
\begin{enumerate}[label=\arabic*)]
  ...
  \item 路径几何验收证明了规划难度，但最小动态障碍距离仍应在实体实验中单独记录；
  \item 当前实地实验没有数据，任何成功率、误差、耗时和安全结论均不得提前填写。
\end{enumerate}

\subsection*{6.6 实地实验分析模板}

完成实地实验后，应至少从以下方面比较仿真与实测：
\begin{enumerate}[label=\arabic*)]
  \item 地图尺度、障碍位置和膨胀半径是否一致；
  \item AMCL 静止漂移、运动漂移和重定位时间；
  \item A* 路径长度、拐点数量及与仿真路线的差异；
  \item DWB 实际速度、急停次数、振荡和人工接管；
  \item 最终位置与航向误差；
  \item 多次重复实验的成功率和耗时分布；
  \item 网络延迟、丢包、时间同步和传感器异常对结果的影响。
\end{enumerate}
...
下一阶段的重点是实体 TurtleBot4 实验。应先以安全和数据完整性为目标，完成网络、
急停、低速短路线和传感器检查，再执行正式路线。实地数据与仿真结果应并列呈现，
对误差和失败进行解释，而不应只保留成功的一次运行。
